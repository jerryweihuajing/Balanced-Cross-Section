# -*- coding: utf-8 -*-
"""
Created on Tue Nov  6 14:38:10 2018

@author:Wei Huajing
@company:Nanjing University
@e-mail:jerryweihuajing@126.com

@title：基于面积守恒的平衡恢复函数库
"""
...
#    print(rgb_list_temp)
    ...
    tag_list=[index+1 for index in range(len(rgb_list_temp))]
    
    #临时的tag_color索引
    rgb_dict_temp=dict(zip(tag_list,rgb_list_temp))
...
#    print(layer_tag_list)
    ...
    if show:
        ...
        plt.figure()
        plt.imshow(Tag2RGB(new_img_tag,rgb_dict))
...
#        plt.imshow(Tag2RGB(final_img_tag,rgb_dict)) 
    ...
    if show:
        ...
        for this_final_img_tag in final_img_tag_list:
            ...
            plt.figure()
            plt.imshow(Tag2RGB(this_final_img_tag,rgb_dict)) 
...
#    plt.close() 
    ...
    for this_final_img_tag in final_img_tag_list:
        ...
        #先转为rgb矩阵
        this_final_img_rgb=Tag2RGB(this_final_img_tag,rgb_dict)
...
def PrintSubplot(img_rgb,final_img_tag_list,rgb_dict,save_path,show=False):
    ...
    #最终要打印的rgb矩阵组成的列表
    final_img_rgb_list=[img_rgb]+[Tag2RGB(this_final_img_tag,rgb_dict) for this_final_img_tag in final_img_tag_list]
